...
/**
 * Game Entity - Represents a Rotle game session
 * Aggregate root for the Rotle Game bounded context
 */
@Entity
@Table(name = "games")
public class Game {
    ...
    public Game(String targetWord, Long targetWordId, LocalDate gameDate, String userSession) {
        ...
        if (targetWord.length() != 5) {
            throw new IllegalArgumentException("Target word must be exactly 5 characters");
        }
        if (targetWordId == null) {
            throw new IllegalArgumentException("Target word ID cannot be null");
        }
        if (gameDate == null) {
    ...
    public Attempt makeAttempt(String guess) {
        ...
        if (guess.length() != 5) {
            throw new IllegalArgumentException("Guess must be exactly 5 characters");
        }
